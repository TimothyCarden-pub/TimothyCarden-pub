# bash script to audit Kubernetes pods for liveness probes and report which pods in which namespace had probes in their spec
...
for namespace in `kubectl get namespaces | grep -v NAME | cut -d ' ' -f 1` ; do
  for pod in `kubectl get pods -n $namespace | grep -v NAME | cut -d ' ' -f 1` ; do
        liveliness_check=`kubectl get pods $pod -n $namespace -o yaml | grep livelinessProbe`
